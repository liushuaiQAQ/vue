...
const name = defaultSettings.title || 'madongdong' // page title
...
function deleteOtherDir() {
	...
	try {
		let webapp_paths = fs.readdirSync(path.join(process.cwd(), 'm'));
		...
		if (webapp_paths.length > RetainVersionNum) {
			...
			delete_webapp_paths.forEach(delete_webapp_path => {
				let delete_webapp_abs_path = path.join(process.cwd(), 'm', delete_webapp_path);
